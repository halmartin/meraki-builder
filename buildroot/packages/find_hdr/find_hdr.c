...
int main( int argc, char *argv[] )  {
   ...
   bool footer = false;
   int opt;
   while ((opt = getopt(argc, argv, "gxaf")) != -1) {
       switch(opt) {
           case 'g': gzip = true; break;
           case 'x': xz = true; break;
           case 'a': sw_arm = true; break;
           case 'f': footer = true; break;
           default:
               fprintf(stderr, "Usage: %s [-gxaf] [file...]\n", argv[0]);
               exit(EXIT_FAILURE);
       }
   }
   if (optind < argc) { 
	   ...
	   // gzip has no footer, so the option is a NOP for gzip
	       ...
	       // from 2.1.1.1 in https://tukaani.org/xz/xz-file-format.txt
               ...
               // "switch-arm Ramdisk"
               uint8_t shdr[18] = {0x73, 0x77, 0x69, 0x74, 0x63, 0x68, 0x2d, 0x61, 0x72, 0x6d, 0x20, 0x52, 0x61, 0x6d, 0x64, 0x69, 0x73, 0x6b};
               le_pos = le_find(file, shdr, sizeof(shdr), footer);
               le_pos = le_pos + 32;
       } else {
           fprintf(stderr, "Usage: %s [-gxaf] [file...]\n", argv[0]);
